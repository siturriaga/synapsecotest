...
function normalizeRemoteBase(raw: string): RemoteBaseConfig {
  ...
  const functionBase = (() => {
    ...
    if (hasApiPrefix) {
      return withoutTrailingSlash.replace(/\/api(?:\b|\/)/i, '/.netlify/functions')
    }
    return withoutTrailingSlash
  })()
  ...
  const apiBase = (() => {
    ...
    if (/\/\.netlify\/functions(?:\b|\/)/i.test(withoutTrailingSlash)) {
      return withoutTrailingSlash.replace(/\/\.netlify\/functions(?:\b|\/)/i, '/api')
    }
...
const REMOTE_BASE_CONFIG = normalizeRemoteBase(RAW_REMOTE_FUNCTION_BASE)
const REMOTE_FUNCTION_BASE = REMOTE_BASE_CONFIG.functionBase
const REMOTE_API_BASE = REMOTE_BASE_CONFIG.apiBase
const REMOTE_BASE_INCLUDES_FUNCTION_PREFIX = REMOTE_BASE_CONFIG.includesFunctionPrefix
